...
**The Archive**:
A Module only the Archivists build, at one Colony off Earth and at most one per Faction, three turns to raise from its own button. Since version 0.08.0 it may be ORDERED only when two things hold, both checked once at the order and never again: the Archivists' gate Tech, **The Upload**, must stand, and **four Colonists** must live at the place. It wants **125 Research** in all, paid into the Archive fund at any pace and, since version 0.09.1, payable in full before the Module stands; complete, with every point paid, it takes a great deal of Energy to keep running, and only then may Colonists be Uploaded into it. That 125 is one figure with the Archivists' Victory bar: the Research the Archive costs is the Research their first Victory part is scored against, and the two are written in two files that the game refuses to load out of step. From version 0.05.5 until 0.09.1 it wanted 80 and the fund held only a quarter of that until the Module stood, which capped their Victory score at a quarter however much they banked. It is destroyed outright if its Colony changes hands, and the fund is kept.
