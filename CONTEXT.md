...
**Archivists**:
The Faction that means to save what humanity knows, and as many of its people as it can, in one place off Earth. Their signature rule is Provisional Findings, their Unique Facility is the Reactor, and they win by completing the Archive and Uploading twelve Colonists into it. From version 0.08.0 until 0.09.1 they could not begin the Archive until The Upload, their own gate Tech, stood. Since version 0.09.1 they may order it as soon as four Colonists live at a Colony of theirs on another Body. The Upload now gates only their win, so their Labs' Research is still a choice between the monument and the tree that opens its door.
...
**The Archive**:
A Module only the Archivists build, at one Colony off Earth and at most one per Faction, three turns to raise from its own button. It may be ORDERED once **four Colonists** live at the place, checked once at the order and never again. From version 0.08.0 the Archivists' gate Tech, **The Upload**, had to stand before the order as well; since version 0.09.1 it does not, and The Upload gates only their win, as every Faction's gate Tech does. It wants **125 Research** in all, paid into the Archive fund at any pace and, since version 0.09.1, payable in full before the Module stands; complete, with every point paid, it takes a great deal of Energy to keep running, and only then may Colonists be Uploaded into it. That 125 is one figure with the Archivists' Victory bar: the Research the Archive costs is the Research their first Victory part is scored against, and the two are written in two files that the game refuses to load out of step. From version 0.05.5 until 0.09.1 it wanted 80 and the fund held only a quarter of that until the Module stood, which capped their Victory score at a quarter however much they banked. It is destroyed outright if its Colony changes hands, and the fund is kept.
_Avoid_: library, vault, monument, database, stage

**Fund the Archive**:
The Archivists' standing declaration that the Research their own Labs make goes into their Archive fund instead of the shared Tech, where it counts nothing toward the Research Lead. Since version 0.07.0 it is set by an order and read at the next Income, before a point of Research reaches the Tech, and it holds until it is set again; the Research is never paid to the Tech and taken back. The fund is capped at what the Archive requires -- nobody banks more Research than the Archive needs -- and since version 0.09.1 it holds the whole of that from the first turn, where until then it held only a quarter of it until the Module stood. What the fund has no room for goes on to the shared Tech, and at a full fund the declaration is refused.
...
**Body**:
A place in the solar system that can hold a Colony or a Space Station. Six since version 0.06.0: Earth (its Colony Slots are Antarctica's, shut under the ice until +1.6 C), the Moon, Mars, Phobos, Deimos and Venus, a Body of orbits only, with no Colony Slots and three Orbital Slots. A Body may be another's satellite, which sets how far apart they are. Since version 0.09.1 the Moon, Mars, Phobos and Deimos each carry a **First to a Body** prize for the Faction that settles its ground before anybody else, larger the further out the Body lies. Earth carries none, and neither does Venus, which has no ground to settle.
...
**Tech Tree**:
The single tree of Techs shared by all Factions: twenty-one since version 0.09.1, which added Missile Technology (twenty from 0.08.3, eighteen from 0.08.1, seventeen from 0.06.0), in five branches: Industry, Propulsion, Off-world Living, Extraction, Society. A branch may hold more than one Tech on a rung; since version 0.07.4 two such Techs stack in a taller branch row on every rung but the last, where they sit side by side, so every column is one box wide. One Tech is under research at a time, worldwide. Costs run 18, 32 and 48 by rung since version 0.08.3, with Coastal Engineering at 15 the one exception. Four of the twenty-one are Victory gates, one per Faction on rung 3: each is a Tech for everyone, and its Faction cannot win until it stands. Since version 0.07.6 a box chosen this turn but not yet committed wears a paler amber than the settled one, reads `chosen`, and has its own Pick button withdrawn.
_Avoid_: per-faction tree, research tree

**Research Lead**:
The Faction that contributed the most Research to the Tech that just completed. It chooses the next Tech, from version 0.07.0 out of a drawn shortlist of three rather than out of everything available; the draw always carries the Lead's own Victory gate once its prerequisites are met, and since version 0.09.1 the next rung toward it until then, so a Faction can be denied a rival's gate but never its own. The first Tech of the game is not drawn for: it is a free choice from the whole of rung 1. Decided afresh for every Tech, and the race for it stands in the top bar as one bar of the four Factions' contributions in their own colours. Since version 0.07.0 the turn cannot end while a human Lead owes the table a Tech, and Research banked while nothing is under research keeps its owner. Since version 0.07.6 a human Lead's pick is **provisional until the turn ends**: pressing a box records the choice and spends nothing, another box replaces it as often as the Lead likes, and ending the turn commits it -- at which point the Shortlist is thrown away, the banked Research pours in and the Tech may complete. A computer seat commits the instant it picks.
_Avoid_: science leader, tech leader

**Shortlist**:
The three Techs the Research Lead chooses between, drawn when a Tech completes, since version 0.07.0. The Lead's own Victory gate is always on it where its prerequisites are met. Since version 0.09.1, until then, it carries the next rung of the Lead's own chain instead: the cheapest Tech still standing between the Lead and its gate. The two are never owed at once, so one place of three is reserved. The rest come from the game's own generator. An empty shortlist is a free choice of everything available, which is how the game opens. Since version 0.07.6 it is kept for as long as a human Lead's pick can still be changed and thrown away only when the pick is committed, since redrawing it on every change would be a free reroll.
...
**Habitat**:
The Module that houses Colonists: four each since version 0.08.1, eight with Expanded Habitats, half again for the Arkwrights, the same on every world; the Core Module's four are separate. Since version 0.09.1 a Habitat that is shut -- mothballed, or dark for want of Energy -- still houses its people, and while more live at the Colony than its working Habitats and Core could hold, the Colony makes everything but Energy at half, once however many are shut. A spare Habitat standing empty may be mothballed for nothing.
_Avoid_: dome, dormitory, quarters, housing module
...
**Home Region**:
The Region a Faction starts in and holds from turn one: the player's own pick, and for each computer seat the highest Industry Level Region still free and not next to a taken one, ties by population (the richest free Region was tried in version 0.09.2 and reverted on the sweep's figures). Since version 0.09.2 every home Region starts with the same Facilities, a Power Plant, a Factory, a Mine, a Refinery and the Launch Site, in the Faction's own versions, whatever the Region's card lists; the card's list is what a neutral Region stands with. Its Standing for the holder begins at the Region's threshold.
_Avoid_: start state, capital, homeland, starting position

**Ship card**:
The selection card of one Ship, since version 0.09.2, opened from its row on the stack's card, from its row in the Roster, or from its label on the Solar System Map while it flies. It holds everything that is one Ship's -- its strength, damage and cargo, its Tank, its Transit and change-of-orbit moves, its loading and unloading, its Bombard, Launch and Rearm, and its repair -- and stands in place of the stack's card with a link back to it. Its moves are one drop-down per Body, nested as the sky is, the Moon under Earth and Phobos and Deimos under Mars, each holding a line per orbit. A Ship in flight has one too, for its line, where it is bound and its Tank, with no moves. The stack's card keeps only what is the whole stack's: its Ships grouped by orbit, the stance, Attack and the moves that send every Ship that can pay. Before this every one of these stood on the stack's card once per Ship.
...
**Missile Carrier**:
The fifth Ship type, since version 0.09.1: the hull that carries one Warhead and fires it with a Launch. Dearer than a Battleship and as frail as a Colony Ship -- no strength, no Pursuit, three hit points -- it cannot land a blow, and it waits on Missile Technology, the first Tech that gates a Ship. It is a Ship but never a warship: it holds no Orbital Control, it blockades nothing, and under the escort rule it is struck only once its party has no warship left standing. On Intercept it opens the Battle as any hull does, and strikes nothing in it. That frailty is the counter to it by decision, and the game has no anti-missile rule.
...
**Rearm**:
The Orders-phase order, since version 0.09.1, that loads a fresh Warhead into a Missile Carrier that has fired: at a Colony or station of its own Faction with a working Shipyard, the hull in that place's own orbit, queued in the yard like a build for 40 Materials and 8 Widgets -- most of a second hull, so that firing is a commitment. It is the Ship's one order of the turn.
_Avoid_: reload, resupply, refit
...
**Research Lab**:
The Facility that makes Research on Earth: two a turn at its base, multiplied by its Region's population weighted by Education, by the Education Level again, by the Faction's Research multiplier and by any Tech or card that reaches it, rounded down. Because Education weighs the people and then multiplies on its own, a small well-schooled Region can out-research a vast one. Since version 0.09.1 its hover is the whole chain of that arithmetic, step by step, the rounding last. A Lab in a Region nobody directs runs itself and pays half into the shared Tech for no one.
_Avoid_: laboratory, science building, observatory (that is the Module off Earth)
...
**Chorus**:
The Arkwrights' Unique Module since version 0.08.3, replacing the Relay at the common price and carrying everything a Relay carries, Relay Networks included. Its clause is **one more Influence in its holder's Allotment for every six Colonists at its own Colony, rounded down** — its own Colony, as an Observatory reads its own Colony's people, not its whole Body. The more people stand there, the further the voice carries, which is the Faction whose entire game is moving people. Until version 0.09.1 the card showed this clause, and Relay Networks, but the Allotment never paid either. Since 0.09.1 both are paid. The per-Colonist point is paid at face value, outside the Faction multiplier, on the Spaceport's argument: moving and keeping people is not diplomacy. The Chorus's base 1 and Relay Networks' +1 stay inside the multiplier, as a Relay's do. It adds nothing to **Standing**: "+1 Influence" has meant the Allotment, the Faction's diplomatic budget everywhere, since version 0.08.3 settled it on the Relay.
...
**Pressed**:
Said of a place a Faction holds, since version 0.09.1, when any rival with a Standing there stands within 10 of the holder's own. Being Pressed is not being about to lose it: the rival still needs the holder's Standing plus the challenge margin, and at least the threshold, to take it. It is measured against the holder's Standing and not against the rival's price, so that it comes early, since a turn's Allotment can cover 10 at once. The player's Pressed places are listed under the Influence slider in the **Command Cluster**, whatever is selected, as *Egypt is Pressed by a rival.*, three lines and then *and N more*, never naming the rival.
...
**Allotment**:
The amount of Influence a Faction receives each turn, split freely across any number of targets during the Orders phase. It is a base, plus the Influence value of every Region the Faction controls (since version 0.03 each state carries its own value, from its economic and military weight), plus what its working Embassies, Relays and Choruses add, all times the Faction multiplier, and it does not carry over. Three things are added after the multiplier at face value: what its Spaceports earned lifting Pioneers off Earth last turn (version 0.08.0); since version 0.09.1 the **First to a Body** windfall and standing +1; and since version 0.09.1 the Chorus's point for every six Colonists.
...
**Orders**:
The phase in which a Faction commits everything it will do that turn: building, raising Industry Level, allocating the Influence Allotment, lifting, setting transits and Stances, and ordering attacks and landings. Any order can be cancelled and fully refunded until End Turn is pressed.
...
**Report**:
The phase that opens a turn for the player, and the dated dispatch it shows. It opens with a headline: the most serious thing that happened, chosen by a fixed order of severity. Everything else is grouped under five headings, In space, On Earth, The climate, Ships and Your works (Ships since version 0.09.2, holding every line about a Ship, among them the one that says the player's Colonists are still waiting aboard off Earth), and an empty heading is left out. Every line that is about somewhere is a way there. It ends with what each rival Faction did, told in plain sentences of what the board could see. It speaks only of what is worth a line: since version 0.07.6 migration is reported **once per Region, in net**, and only where the net is worth at least half a person. Since version 0.09.1 it tells a change in Relations only where the player is one of the pair and the pair has fallen into a worse named level, one folded line each way.
...
**Emissions**:
The CO2-equivalent a source adds to the CO2 Stock in a turn. Every source has its own figure and the Climate Panel shows them one by one before the sum; methane-heavy sources carry a heavier weight. Since version 0.05 a Region's people emit more the more built-up the state is, and a Leapfrog lowers that state's figure for good. Since version 0.08.5 war is a source: every hit landed in a Battle on Earth or in Earth orbit, every building burned when a place is taken by force, and since version 0.09.1 a flat 20 ppm for every Launch on or over Earth, puts ppm in the air at the next Climate phase, worn as Blame by whoever landed the hit or made the taking, nobody's for a neutral Region's own Army, and counted against a Stabilization run. A Battle at any other Body fouls nobody's air.
...
**Break**:
A Temperature at which a permanent change to the world fires once, the first time the Temperature stands at or above it. Five since version 0.05: the reefs die, the permafrost thaws, the Natural Sink weakens, the ice sheets go and the Amazon dies back. Since version 0.09.1 the Sink Weakens **subtracts** 2 ppm from the Natural Sink, never below nothing, instead of assigning it a figure; on an untouched world it lands where it always did, but it no longer erases whatever had raised the Sink first. Nothing undoes a Break, and the Report says it happened rather than that it is coming.
