...
| [`../../playtest/PLAYTEST.txt`](../../playtest/PLAYTEST.txt) | [Write the 0.08.6 amendments and build them](https://github.com/whaleyjoshua2/Dying-Earth/issues/301) | The playtest note that ships in the Windows kit as `README.txt` (this version ships for Windows only, at the designer's word). It lives at `docs/playtest/` while this is the current version, because the kit workflow copies it from there; 0.08.5's is filed under [`playtest/`](../2026-09-20-version-0.08.5/playtest/PLAYTEST.txt) in its own folder. |
