...
and the Factory Module, `CancelBuild`, SAVE_VERSION 3) is the commit at `9071342`; `SPEC.md` beside
this file is its specification. This is the presentation the designer's resolution asked for under
*The player reads Widgets*, *The glyph* and *A place that changes hands*.

| picture | what it shows |
|---|---|
| [`widgets-region-earth.png`](widgets-region-earth.png) | `shot: queue:1 order:powerplant select:eastasia panel:0 window:1600x1000`. The top bar with the **cog** after the Materials' cart, reading `13 / 0`: thirteen Widgets a turn across the three places the Custodians direct (China 7, the ISS 1, a Moon Colony 5), nought applied because no Resolution has run. China's card: **Widgets 7 a turn** with the queue under it in order -- *Power Plant 3 of 8, ready next turn; Research Lab 0 of 4, about 2 turns; Bank 2 of 8, about 3 turns, begun by the Prospectors* -- and the hatched boxes wearing `3 of 8`, `0 of 4` and `2 of 8` in place of the turns, with the Power Plant ordered this turn hatched at `0 of 8`. The Scrubber button reads `30 [cart] 8 [cog]`. |
| [`widgets-cancel-earth.png`](widgets-cancel-earth.png) | `shot: queue:1 select:eastasia panel:0 tip:"Right-click to cancel it"`. The hover on the Prospectors' Bank, a build left behind when the Region changed hands: *Bank (inland): building, 2 of 8 Widgets, about 3 turns at this place's rate. Begun by the Prospectors, who paid for it. Right-click to cancel it: 25 Materials to your Stockpile at your own price, and its Widgets so far are lost.* The right-click gives `Order::CancelBuild`. |
| [`widgets-colony-moon.png`](widgets-colony-moon.png) | `shot: queue:1 hab:ground habtile:free tip:"turns here" window:1280x1300`. A Colony card: **Widgets 5 a turn** (the Core Module's one and a Factory Module's four), the queue of three under it, the tiles `2 of 4`, `0 of 4` and `1 of 8`, and the Build here strip with every Module's face reading Materials then Widgets -- `Mine 20 [cart] 4 [cog]`, down to the new **Factory 20 [cart] 4 [cog]** last in the list -- and the Mine's hover: *20 Materials, 4 Widgets, about 4 turns here. Once it stands: +6 Materials, 3 Energy upkeep.* Four turns is right: seventeen Widgets owed ahead of and including it at five a turn. Build Army (Barracks) carries its cog too. |
| [`widgets-buttons-earth.png`](widgets-buttons-earth.png) | `shot: select:eastasia slotbox:free panel:0 tip:"turns here" window:1280x1500`. A fresh China with a free box clicked: the Facility list with the new **Mine 20 [cart] 4 [cog]** in it, every face priced in both halves, the Power Plant's hover *25 Materials, 8 Widgets, about 2 turns here. Once it stands: +6 Energy, 1.1 Emissions*, and below the boxes `Raise Industry Level 30 [cart] 4 [cog]` and `Build Army 25 [cart] 4 [cog]`. The bar reads `8 / 0` here: China 7 and the ISS 1, no Colony planted. |
| [`widgets-bar-solar.png`](widgets-bar-solar.png) | `shot: queue:1 tip:"across the places"`. The top bar's Widgets hover: the two figures explained, the rule in three sentences, then a line per directed place -- *China: 7 a turn (3 from Industry Level 3, 4 from the Factory). Power Plant 3 of 8, ready next turn; ...*, *ISS over Earth: 1 a turn (1 from the Core Module). nothing under way.*, and the Moon Colony's. |
| [`widgets-underway-solar.png`](widgets-underway-solar.png) | `shot: queue:1 factions:1`. The Faction window's Under way block: *Building: a Habitat at Mare Tranquillitatis on the Moon (2 of 4, ready next turn), a Power Plant in China (3 of 8, ready next turn), a Mine at ... (0 of 4, about 2 turns), a Research Lab in China (0 of 4, about 2 turns)*, soonest first; the Prospectors' two builds are not the Custodians' and are not listed. |
...
- **Words**: the Faction rulebook's discount rows say `every Ship x0.85 [cart] and [cog]`,
  ...
  tutorial's turn-2 and turn-4 notes said a Habitat on the ISS was ready next turn and that all
  four Pioneers would fit on turn 4; at one Widget a turn from the Core Module a four-Widget
  Habitat stands on turn 6, so the notes now say what fits when. The chain's timing is a design
  question left for the designer.
